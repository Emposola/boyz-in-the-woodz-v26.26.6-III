...
const MOBILE_NAV = [
  ...
  { label: 'Experience', icon: Trees, to: '/services' },
  { label: 'Brotherhood', icon: Users, to: '/about' },
  { label: 'Dashboard', icon: LayoutDashboard, to: '/account' },
];

const MARQUEE = ['🌲 Survival Pack 01 — Now Available','🏕️ Broken Bow Fall Retreat — 4 Spots Left','⭐ Earn Points Per $1 Spent','🎁 Free Shipping Over $99','🔥 Virtual Campfire — Thu 8PM EST','🌿 Brotherhood. Freedom. Nature.'];
...
export default function Navbar() {
  ...
  return (
    <>
      {/* Marquee */}
      <div className="overflow-hidden py-1.5 bg-white text-background">
        <div className="flex whitespace-nowrap animate-marquee">
          {[...MARQUEE, ...MARQUEE].map((item, i) => (
            <span key={i} className="text-xs font-heading tracking-widest uppercase px-8 text-background">{item}</span>
          ))}
        </div>
      </div>
      ...
      <div className="sticky top-3 z-50 px-4">
        <nav className="max-w-7xl mx-auto rounded-[32px] px-6 h-14 flex items-center justify-between"
          ...
          <Link to="/" className="flex items-center gap-3 flex-shrink-0">
            <img src={LOGO_URL} alt="BITW" className="h-9 w-auto rounded-xl" />
            <div className="hidden sm:block">
              <div className="font-heading text-[19px] leading-none tracking-wider" style={{ color: FG }}>BOYZ IN THE WOODZ</div>
              <div className="text-[10px] tracking-widest" style={{ color: SAND }}>Brotherhood. Freedom. Nature.</div>
            </div>
          </Link>
        ...
        <AnimatePresence>
          {openMenu && (
            <motion.div
              ...
              className="absolute left-0 right-0 mx-auto max-w-7xl mt-0"
              style={{ top: '56px' }}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            className="fixed top-24 left-4 right-4 z-40 rounded-2xl overflow-hidden shadow-2xl"
      ...
      {/* Mobile Bottom Nav */}
      <div className="lg:hidden fixed bottom-3 left-4 right-4 z-50 rounded-[32px] shadow-2xl overflow-hidden"
