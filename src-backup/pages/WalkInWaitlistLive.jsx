...
export default function WalkInWaitlistLive() {
  ...
  const { data: queue, isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.WaitlistQueue.filter({ status: 'waiting' }, 'created_date', 50),
    initialData: [],
  ...
  useEffect(() => {
    const unsub = base44.entities.WaitlistQueue.subscribe(() => {
  ...
  const joinMutation = useMutation({
    mutationFn: () => base44.entities.WaitlistQueue.create({
      ...form,
  ...
  const leaveMutation = useMutation({
    mutationFn: () => base44.entities.WaitlistQueue.update(myEntry.id, { status: 'left' }),
    onSuccess: () => {
      setMyEntry(null);
