...
export default function RetreatApply() {
  ...
  const { data: events } = useQuery({
    ...
    queryFn: () => base44.entities.Event.filter({ type: 'retreat', active: true }, 'start_date', 10),
    initialData: [],
  });
  ...
  const handleSubmit = async () => {
    if (!user) { base44.auth.redirectToLogin('/retreat/apply'); return; }
    setSubmitting(true);
    await base44.entities.RetreatApplication.create({
      user_id: user.id,
