...
export default function RetreatConfirmed() {
  ...
  const { data: applications } = useQuery({
    ...
    queryFn: () => base44.entities.RetreatApplication.filter({ user_id: user.id, status: 'confirmed' }, '-created_date', 1),
    enabled: !!user?.id,
    initialData: [],
  });
  ...
  const handleSignWaiver = async () => {
    ...
    if (app) {
      await base44.entities.RetreatApplication.update(app.id, {
        waiver_signed: true,
